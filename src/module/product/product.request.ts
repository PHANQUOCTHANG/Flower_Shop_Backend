...
const productBase = z.object({
  ...
  comparePrice: z.number().min(0, "Giá so sánh không được âm").nullable().optional(),
  ...
  stockQuantity: z.number().int().min(0, "Số lượng kho không được âm").default(0),
...
// [Schema] Kiểm tra ID trên URL (BigInt)
export const ProductIdParamSchema = z.object({
  id: z.string().regex(/^\d+$/, "ID sản phẩm phải là số hợp lệ"),
});
